...
async def start_program(program_name: str, current_user: str = Depends(get_current_user)):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to start program {program_name}: {str(e)}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def stop_program(program_name: str, current_user: str = Depends(get_current_user)):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to stop program {program_name}: {str(e)}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
    ...
    try:
        async with mysql_pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute("""
                    SELECT country, COUNT(*) as sms_sent, AVG(success) as success_rate 
                    FROM sms_logs 
                    GROUP BY country
                """)
        ...
        metrics = [
            ...
            for country, sms_sent, success_rate in result
        ]
        return metrics
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
